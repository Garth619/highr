...
class wpseo_Video_Sitemap {
	...
	function register_settings_page() {
		add_submenu_page( 'wpseo_dashboard', __( 'Video SEO', 'yoast-video-seo' ), __( 'Video SEO', 'yoast-video-seo' ),
	...
	/**
	 * Pings Bing & Google with the (presumeably updated) Video Sitemap.
	 *
	 * @since 0.1
	 */
	function ping() {
		wp_remote_get( 'http://www.google.com/webmasters/tools/ping?sitemap=' . urlencode( $this->sitemap_url() ) );
		wp_remote_get( 'http://www.bing.com/webmaster/ping.aspx?siteMap=' . urlencode( $this->sitemap_url() ) );
	}

	/**
	 * Updates the last update time transient for the video sitemap and pings search engines with the sitemap.
	 *
	 * @since 0.1
	 */
	 ...
	 */
	function make_image_local( $url, $vid ) {
	...
	function vippy_details( $vid, $oldvid = array(), $thumb = '' ) {
		...
		$vid['type']        = 'vippy';
		$vid['player_loc']  = isset( $vippy_video->open_graph_url ) ? $vippy_video->open_graph_url : '';
	...
	function get_meta_boxes() {
		...
		$mbs = array(
			...
			array(
				...
				"description" => ( isset( $video['thumbnail_loc'] ) && !get_post_meta( $post->ID, 'videositemap-thumbnail', true ) ) ? sprintf( __( 'Now set to %1$sthis image%2$s based on the embed code.', 'yoast-video-seo' ), '<a target="_blank" href="' . $video['thumbnail_loc'] . '">', '</a>' ) : '',
			),
			array(
