...
if(supportedBrowser){window.addEventListener('message',window.wp.receiveEmbedMessage,!1);document.addEventListener('DOMContentLoaded',onLoad,!1);window.addEventListener('load',onLoad,!1)}})(window,document);(function($){'use strict';var escape=/["\\\x00-\x1f\x7f-\x9f]/g,meta={'\b':'\\b','\t':'\\t','\n':'\\n','\f':'\\f','\r':'\\r','"':'\\"','\\':'\\\\'},hasOwn=Object.prototype.hasOwnProperty;$.toJSON=typeof JSON==='object'&&JSON.stringify?JSON.stringify:function(o){if(o===null){return 'null'}
...
return '"'+str+'"'}}(jQuery));if(typeof jQuery.fn.prop==='undefined'){jQuery.fn.prop=jQuery.fn.attr}
...
return "";number=number+"";negative="";if(number[0]=="-"){number=parseFloat(number.substr(1));negative='-'}
...
return d}}
function gformCleanNumber(text,symbol_right,symbol_left,decimal_separator){var clean_number='',float_number='',digit='',is_negative=!1;text=text+" ";text=text.replace(/&.*?;/g,"");text=text.replace(symbol_right,"");text=text.replace(symbol_left,"");for(var i=0;i<text.length;i++){digit=text.substr(i,1);if((parseInt(digit)>=0&&parseInt(digit)<=9)||digit==decimal_separator)
...
price=window.gform_product_total(formId,price);price=gform.applyFilters('gform_product_total',price,formId);var totalElement=jQuery(".ginput_total_"+formId);if(totalElement.length>0){var currentTotal=totalElement.next().val(),formattedTotal=gformFormatMoney(price);if(currentTotal!=price){totalElement.next().val(price).change()}
if(formattedTotal!=totalElement.first().text()){totalElement.html(formattedTotal)}}}
...
price=price*quantity;price=Math.round(price*100)/100;return price}
function gformGetProductQuantity(formId,productFieldId){if(!gformIsProductSelected(formId,productFieldId)){return 0}
var quantity,quantityInput=jQuery('#ginput_quantity_'+formId+'_'+productFieldId),numberFormat;if(quantityInput.length>0){quantity=quantityInput.val()}else{quantityInput=jQuery('.gfield_quantity_'+formId+'_'+productFieldId+' :input');quantity=1;if(quantityInput.length>0){quantity=quantityInput.val();var htmlId=quantityInput.attr('id'),fieldId=gf_get_input_id_by_html_id(htmlId);numberFormat=gf_global.number_formats[formId][fieldId]}}
if(!numberFormat)
...
function gformIsProductSelected(formId,productFieldId){var suffix="_"+formId+"_"+productFieldId;var productField=jQuery("#ginput_base_price"+suffix+", .gfield_donation"+suffix+" input[type=\"text\"], .gfield_product"+suffix+" .ginput_amount");if(productField.val()&&!gformIsHidden(productField)){return !0}
...
function gformInitPriceFields(){jQuery(".gfield_price").each(function(){var productIds=gformGetProductIds("gfield_price",this);gformRegisterPriceField(productIds);jQuery(this).on('change','input[type="text"], input[type="number"], select',function(){var productIds=gformGetProductIds("gfield_price",this);if(productIds.formId==0)
...
var $group=$addButton.parents('.gfield_list_group'),$clone=$group.clone(),$container=$group.parents('.gfield_list_container'),tabindex=$clone.find(':input:last').attr('tabindex');$clone.find('input, select').attr('tabindex',tabindex).not(':checkbox, :radio').val('');$clone.find(':checkbox, :radio').prop('checked',!1);$clone=gform.applyFilters('gform_list_item_pre_add',$clone,$group);$group.after($clone);gformToggleIcons($container,max);gformAdjustClasses($container)}
function gformDeleteListItem(deleteButton,max){var $deleteButton=jQuery(deleteButton),$group=$deleteButton.parents('.gfield_list_group'),$container=$group.parents('.gfield_list_container');$group.remove();gformToggleIcons($container,max);gformAdjustClasses($container)}
...
numberFormat=gf_global.number_formats[formId][formulaField.field_id];var decimalSeparator=gformGetDecimalSeparator(numberFormat);value=gform.applyFilters('gform_merge_tag_value_pre_calculation',value,matches[i],isVisible,formulaField,formId);value=gformCleanNumber(value,'','',decimalSeparator);if(!value)
...
function gf_get_input_id_by_html_id(htmlId){var ids=gf_get_ids_by_html_id(htmlId),id=ids[2];if(ids[3]){id+='.'+ids[3]}
return id}
function gf_get_ids_by_html_id(htmlId){var ids=htmlId?htmlId.split('_'):!1;return ids}
